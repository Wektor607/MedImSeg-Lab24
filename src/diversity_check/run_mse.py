...
def compute_mse(cluster_centers, embeddings):
    """
    Compute the Mean Squared Error (MSE) between cluster centers and embeddings.
    """
    cluster_distances = np.linalg.norm(cluster_centers[:, None, :] - cluster_centers[None, :, :], axis=-1)
    embedding_distances = np.linalg.norm(embeddings[:, None, :] - embeddings[None, :, :], axis=-1)
    mse = mean_squared_error(cluster_distances.flatten(), embedding_distances.flatten())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint_path', type=str, default='/home/chopra/lab-git/MedImSeg-Lab24/checkpoints/mnmv2-15-19_10-12-2024-v1.ckpt', help="Checkpoint path.")
    parser.add_argument('--device', type=str, default='cuda:0', help="Device to use for training.")
    parser.add_argument('--threshold', type=float, default=0.5, help="Confidence threshold.")
    parser.add_argument('--use_uncertainty', type=bool, default=True, help="Whether to use uncertainty weights.")
    ...
    # Compute nearest indices and embeddings
    ...
    if cluster_centers is not None and embeddings is not None:
        mse = compute_mse(cluster_centers, embeddings)
        print(f"[INFO] Number of clusters: {args.num_clusters}, MSE: {mse}")
    else:
        print("[ERROR] Cluster centers or embeddings are None.")

    # Log results
    log_dir = "/home/chopra/lab-git/MedImSeg-Lab24/results/"
    os.makedirs(log_dir, exist_ok=True)
    log_file = os.path.join(log_dir, "mse_results.txt")
    with open(log_file, "a") as f:
        f.write(f"Clusters: {args.num_clusters}, MSE: {mse:.4f}\n")
