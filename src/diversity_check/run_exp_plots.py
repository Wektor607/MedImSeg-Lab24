...
if __name__ == '__main__':
    ...
    for i in [5]:
        ...
        if args.cluster_type == 'centroids':
            save_dir = '../pre-trained/plots/finetuned_on_centroids_plots'
        else:
            save_dir = '../pre-trained/plots/finetuned_on_uncert_points_plots'
...
def get_embedding(model, dataloader, device):
    """
    Extract 2D embeddings for all samples in a dataloader using the model.

    Args:
        model: Trained PyTorch model used for feature extraction.
        dataloader: Dataloader for the dataset from which embeddings are extracted.
        device: The device (CPU/GPU) to perform computations on.

    Returns:
        embeddings (numpy array): Array of extracted 2D embeddings.
        indices (list): List of sample indices corresponding to embeddings.
    """
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader):
            inputs = batch['input'].to(device)  # Move inputs to the same device as the model
            batch_embeddings = model(inputs)  # Forward pass
            
            # Global average pooling to reduce spatial dimensions
            if batch_embeddings.ndim == 4:  # If [batch_size, channels, height, width]
                batch_embeddings = torch.mean(batch_embeddings, dim=(2, 3))  # Pool to [batch_size, channels]
            
            embeddings.append(batch_embeddings.cpu().numpy())  # Move embeddings to CPU
            indices.extend(range(batch_idx * len(inputs), (batch_idx + 1) * len(inputs)))

    embeddings = np.vstack(embeddings)  # Combine all batches into a single array
    return embeddings, indices


def group_indices_by_cluster(embeddings, indices, n_clusters):
    """
    Group indices based on clustering of embeddings.

    Args:
        embeddings: Numpy array of embeddings.
        indices: List of indices corresponding to embeddings.
...
def plot_clustered_images(images, cluster_id, save_dir, grid_size=(5, 5)):
    """
    Plot clustered images in a grid and save the plot.

    Args:
        images (list): List of images (e.g., tensors or numpy arrays).
        cluster_id (int): Cluster ID for naming the plot.
    ...
    for i, ax in enumerate(axes):
        if i < len(images):
            img = images[i].cpu().numpy() if torch.is_tensor(images[i]) else images[i]
    ...
    plt.close(fig)
    print(f"Cluster {cluster_id} plot saved at: {save_path}")

# Main script continuation
# if __name__ == '__main__':
# Load embeddings and indices from the test dataloader
# Load embeddings and indices from the test dataloader
embeddings, indices = get_embedding(model, datamodule.test_dataloader(), device)

# Group indices into clusters
cluster_groups = group_indices_by_cluster(embeddings, indices, n_clusters=args.num_clusters)

# Plot and save clustered images
for cluster_id, cluster_indices in enumerate(cluster_groups):
    cluster_images = [datamodule.mnm_test[idx]['input'] for idx in cluster_indices]
    plot_clustered_images(cluster_images, cluster_id, save_dir='/home/chopra/lab-git/MedImSeg-Lab24/pre-trained/plots')