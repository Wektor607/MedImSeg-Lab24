...
def compute_true_uncertainty_weights(dice_scores):
    return 1.0 - dice_scores  # True uncertainty is 1 - Dice score

def compute_ground_truth_weights(targets):
    class_counts = torch.bincount(targets.view(-1))
    total_samples = targets.numel()
    class_weights = total_samples / (class_counts + 1e-6)
    sample_weights = class_weights[targets.view(-1)].reshape(targets.shape)
...
if __name__ == '__main__':
    ...
    for i in [1, 10, 25, 40, 50, 60, 75, 100, 125, 150, 175, 200]:
        ...
        # Debugging: Check if mnm_train exists
        if not hasattr(datamodule, 'mnm_train') or datamodule.mnm_train is None:
            raise AttributeError("MNMv2DataModule does not have an attribute 'mnm_train'. Ensure setup(stage='train') initializes it correctly.")

        if args.use_true_uncertainty:
            dice_scores = torch.rand(len(datamodule.mnm_train.target))  # Simulated Dice scores
            weights = compute_true_uncertainty_weights(dice_scores)
        elif args.use_ground_truth_weighting:
            weights = compute_ground_truth_weights(datamodule.mnm_train.target)
        else:
        ...
        if args.cluster_type == 'centroids':
            save_dir = '/home/chopra/lab-git/MedImSeg-Lab24/results/cheat/32/'
        else:
            save_dir = '/home/chopra/lab-git/MedImSeg-Lab24/results/cheat/32/'
        ...
        if i == 1:
            with open("/home/chopra/lab-git/MedImSeg-Lab24/results/cheat/32/results_test_100.txt", "w") as f:
                f.write(f"Num_Centroids\tLoss\tDice_Score\tNum_epochs\tCentroid_time\n")    
        
        with open("/home/chopra/lab-git/MedImSeg-Lab24/results/cheat/32/results_test_100.txt", "a") as f:
