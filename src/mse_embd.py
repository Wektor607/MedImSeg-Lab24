...
@
        ...
        # Ensure the test dataset loader is used
        test_loader = datamodule.test_dataloader()
        # Convert dictionary-based DataLoader output to tuple
        test_loader = [(batch["input"], batch["target"]) for batch in test_loader]

        # Extract embeddings and cluster centers
        embeddings = clue_sampler.get_embedding(
            model=model,
            loader=test_loader,  # Pass the correct DataLoader
            device=device,
            args=cfg
        )  # Shape: (n_samples, embedding_dim)

        centroids = clue_sampler.get_cluster_centers()  # Shape: (n_clusters, embedding_dim)

        # Assign each embedding to its nearest centroid
        distances = euclidean_distances(embeddings, centroids)  # Shape: (n_samples, n_clusters)
        nearest_centroid_indices = np.argmin(distances, axis=1)  # Shape: (n_samples,)

        # Get the nearest centroid for each embedding
        nearest_centroids = centroids[nearest_centroid_indices]  # Shape: (n_samples, embedding_dim)

        # Calculate MSE between embeddings and their nearest centroids
        mse = mean_squared_error(embeddings, nearest_centroids)

        # Save MSE to file
        mse_file_path = "/home/chopra/lab-git/MedImSeg-Lab24/results/uniform_weights/32/mse_results_embd_uniform_1_125_tempp.txt"
        with open(mse_file_path, "a") as f:
            f.write(f"Num_Centroids: {i}, MSE: {mse:.4f}\n")
        ...
        if i == 1:
            with open("/home/chopra/lab-git/MedImSeg-Lab24/results/results_test_embd_32_100_uniform_1_125_temp.txt", "w") as f:
                f.write(f"Num_Centroids\tLoss\tDice_Score\tMSE\tNum_epochs\tCentroid_time\n")    
        
        with open("/home/chopra/lab-git/MedImSeg-Lab24/results/results_test_embd_32_100_uniform_1_125_temp.txt", "a") as f:
